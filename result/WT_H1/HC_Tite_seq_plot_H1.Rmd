...
dt <- read.csv(file="./CR9114HC_WT_count_aa_H1.tsv", sep = '\t')
dt[is.na(dt)] <- 0
...
# 处理info_df
info_df <- read_excel("./20241007_Sample_IDs_for_Yiquan_DMS1CR9114WT.xlsx", sheet = 'H1')
info_df <- info_df %>%
  filter(!grepl("pre", `Sample ID_2`))
...
# 创建排序函数
get_sort_values <- function(sample_id) {
  # 提取第二个点后的部分（如"17_28"）
  id_part <- sub(".*\\.([0-9]+_[0-9A-Z]+)$", "\\1", sample_id)
  
  # 分割_前后的部分
  prefix <- as.numeric(sub("_.*", "", id_part))  # 提取_前面的数字
  suffix <- sub(".*_", "", id_part)  # 提取_后面的部分
...
# 对info_df排序
info_sort_values <- do.call(rbind, lapply(info_df$`Sample ID_2`, get_sort_values))
...
dt <- dt %>%
  filter(`2024.07.17_0.003nM_Gateun` >= 1)%>%
  filter(`2024.07.22_0.003nM_Gateun` >= 1)

# calculate bin mean

# concentration list
conc_ls <- c('10nM','3nM','1nM','0.3nM','0.1nM','0.03nM','0.01nM','0.003nM')
# bin list
bin_ls <- c('_Gateun','_Gate1','_Gate2','_Gate3')
# repeat date list
rep_ls <- c('2024.07.17_','2024.07.22_')
...
clean_df$conc[clean_df$conc == '3nM'] <-3e-09
clean_df$conc[clean_df$conc == '1nM'] <-1e-09
clean_df$conc[clean_df$conc == '0.3nM'] <-0.3e-09
clean_df$conc[clean_df$conc == '0.1nM'] <-0.1e-09
clean_df$conc[clean_df$conc == '0.03nM'] <-0.03e-09
clean_df$conc[clean_df$conc == '0.01nM'] <-0.01e-09
clean_df$conc[clean_df$conc == '0.003nM'] <-0.003e-09
