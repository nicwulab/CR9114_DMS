...
dt <- read.csv(file="./CR9114HC_WT_count_aa_fluB.tsv", sep = '\t')
dt[is.na(dt)] <- 0

# 删除带有'pre'的列
dt <- dt[, !grepl("pre|73|74", names(dt))]

# 处理info_df
info_df <- read_excel("./20241007_Sample_IDs_for_Yiquan_DMS1CR9114WT.xlsx", sheet = 'fluB')
info_df <- info_df %>%
...
dt_nums <- as.numeric(gsub("[^0-9]", "", dt_cols))
dt_order <- order(dt_nums)
dt <- dt[, c(1:2, dt_order + 2)]
...
get_sort_values <- function(name) {
  ...
  if(grepl("^[A-Z]$", suffix)) {
    ...
    sort_value <- as.numeric(suffix)
  }
  return(sort_value)
}

# 对info_df排序
info_sort_values <- sapply(info_df$`Sequencing Label`, get_sort_values)
info_df <- info_df[order(info_sort_values), ]
...
dt <- dt %>%
  filter(`2024.07.31_0.067nM_Gateun` >= 1)%>%
  filter(`2024.08.01_0.067nM_Gateun` >= 1)

# calculate bin mean

# concentration list
conc_ls <- c('200nM','67nM','20nM','6.7nM','2nM','0.67nM','0.2nM','0.067nM')
# bin list
bin_ls <- c('_Gateun','_Gate1','_Gate2','_Gate3')
# repeat date list
rep_ls <- c('2024.07.31_','2024.08.01_')
...
# make conc numeric
clean_df$conc[clean_df$conc == '200nM'] <-200e-09
clean_df$conc[clean_df$conc == '67nM'] <-67e-09
# clean_df$conc[clean_df$conc == '30nM'] <-30e-09
clean_df$conc[clean_df$conc == '20nM'] <-20e-09
clean_df$conc[clean_df$conc == '6.7nM'] <-6.7e-09
clean_df$conc[clean_df$conc == '2nM'] <-2e-09
clean_df$conc[clean_df$conc == '0.67nM'] <-0.67e-09
clean_df$conc[clean_df$conc == '0.2nM'] <-0.2e-09
clean_df$conc[clean_df$conc == '0.067nM'] <-0.067e-09
