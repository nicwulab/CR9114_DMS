...
def Output(count_dict, outfile):
  ...
  sampleIDs = [sampleID for sampleID in sorted(count_dict.keys())]
...
def main():
  outfile = 'result/CR9114HC_WT_count_nuc.tsv'
  merged_fastq_files = glob.glob('fastq_merged/*.assembled.fastq')
  count_dict = {}
  for merged_fastq_file in sorted(merged_fastq_files):
    sampleID = merged_fastq_file.rsplit('/')[1].rsplit('.')[0]
